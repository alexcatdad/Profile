...
const resumeData = {
  basics: {
    ...
    profiles: [
      {
        network: 'LinkedIn',
        username: 'alexalexandrescu',
        url: 'https://linkedin.com/in/alexalexandrescu',
      },
  ...
  work: [
    {
      ...
      endDate: '',
      summary:
        'Voice biometric analysis startup using ML for speaker identification and speech content analysis.',
      highlights: [
    ...
    },
    {
      ...
      endDate: '2022-12',
      summary:
        'Founded software development agency specializing in Web3 and modern web technologies.',
      highlights: [
  ...
  skills: [
    ...
    },
    {
      ...
      level: 'Advanced',
      keywords: ['Turborepo', 'Docker', 'Kubernetes', 'AWS', 'Scaleway', 'CI/CD', 'GitHub Actions'],
    },
    {
    ...
    },
    {
      name: 'Data & APIs',
      level: 'Advanced',
      keywords: ['PostgreSQL', 'MySQL', 'MongoDB', 'REST APIs', 'GraphQL'],
    },
    {
  ...
  projects: [
    {
      name: 'Terminal UI for React',
      description: 'React component library — terminal-style UI for web. Inverse of Ink.',
      keywords: ['React', 'TypeScript', 'UI Library', 'Open Source'],
    },
    {
      name: 'Open Source Philosophy',
      description: 'Extract and publish reusable, non-proprietary solutions from client projects.',
      url: 'https://github.com/alexcatdad',
    },
  ],
  volunteer: [
    {
      organization: 'Cat Rescue Network',
      position: 'Rescue Volunteer',
      startDate: '2022-01',
      summary:
        'Transport, foster, socialize cats. Consult with adopters. Coordinate with veterinarians.',
    },
  ],
  publications: [
    {
      name: '10 Years of Remote Work',
      summary: 'Book in research phase — personal experience vs. industry adoption patterns.',
    },
  ],
  interests: [
    {
      name: 'Photography',
      keywords: ['Published photographer', 'Boudoir/Glamour', 'Studios in Bucharest & Berlin'],
    },
    {
      name: 'Vintage Car Mechanics',
      keywords: ['BMW E46 Individual restoration', 'Mitsubishi Lancer Ralliart resurrection'],
    },
    {
      name: 'Homelabbing',
      keywords: ['TrueNAS Scale', 'Self-hosted infrastructure', 'Media servers'],
    },
  ],
  education: [
    {
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              ...
              <p className="text-zinc-400 text-sm mt-1">
                {basics.location.city}, {basics.location.countryCode} · {basics.pronouns}
              </p>
            </div>
                ...
                <div key={skill.name} className="bg-zinc-900 rounded-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skill.keywords.map((kw) => (
                      <span
                        key={kw}
                        className="text-xs px-2 py-1 bg-zinc-800 rounded-full text-emerald-400"
                      >
                        {kw}
                      </span>
                    ))}
        ...
        {(activeTab === 'full' || activeTab === 'experience') && (
          <section className="mb-8">
            ...
            <div className="space-y-6">
              {work.map((job) => (
                <div key={job.name + job.startDate} className="bg-zinc-900 rounded-lg p-4">
                  ...
                  {job.highlights && (
                    <ul className="space-y-1">
                      {job.highlights.map((h) => (
                        <li key={h} className="text-sm text-zinc-300 flex items-start gap-2">
                  ...
                  {job.keywords && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      {job.keywords.map((kw) => (
                        <span
                          key={kw}
                          className="text-xs px-2 py-0.5 bg-zinc-800 rounded text-zinc-500"
                        >
                          {kw}
                        </span>
                      ))}
        ...
        {(activeTab === 'full' || activeTab === 'projects') && (
          <section className="mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {projects.map((proj) => (
                <div key={proj.name} className="bg-zinc-900 rounded-lg p-4">
                  ...
                  {proj.keywords && (
                    <div className="flex flex-wrap gap-1">
                      {proj.keywords.map((kw) => (
                        <span
                          key={kw}
                          className="text-xs px-2 py-0.5 bg-zinc-800 rounded text-emerald-400"
                        >
                          {kw}
                        </span>
                      ))}
        ...
        {(activeTab === 'full' || activeTab === 'projects') && (
          <section className="mb-8">
            ...
            {publications.map((pub) => (
              <div key={pub.name} className="bg-zinc-900 rounded-lg p-4">
        ...
        {(activeTab === 'full' || activeTab === 'personal') && (
          <>
            <section className="mb-8">
              ...
              {volunteer.map((v) => (
                <div key={v.organization + v.startDate} className="bg-zinc-900 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-sm text-zinc-500">
                      {formatDate(v.startDate)} – Present
                    </span>
                  </div>
            ...
            <section className="mb-8">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {interests.map((int) => (
                  <div key={int.name} className="bg-zinc-900 rounded-lg p-4">
                    <h3 className="font-medium text-white mb-2">{int.name}</h3>
                    <ul className="space-y-1">
                      {int.keywords.map((kw) => (
                        <li key={kw} className="text-sm text-zinc-400">
                          {kw}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-zinc-900 rounded-lg p-4">
                  ...
                  <p className="text-zinc-500 text-sm">
                    {education[0].area} · {education[0].endDate}
                  </p>
                </div>
                <div className="bg-zinc-900 rounded-lg p-4">
