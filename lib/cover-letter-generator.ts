  ...
  try {
    ...
    return {
      header: {
        ...
        linkedin: parsed.header?.linkedin || basics.profiles?.find(p => p.network === 'LinkedIn')?.url || '',
      },
...
function formatLocation(location?: { city?: string; countryCode?: string; region?: string }): string {
  if (!location) return '';
