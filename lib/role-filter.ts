import type { JSONResume, Work, Project, Skill } from '@/types/json-resume';
...
function filterWork(work: Work[], role: RoleType): Work[] {
  ...
  return work.filter((job) => {
    ...
    if (job.roles && job.roles.includes(role)) {
      return true;
    }
    return false;
  });
...
function filterProjects(projects: Project[], role: RoleType): Project[] {
  ...
  return projects.filter((project) => {
    ...
    if (project.roles && project.roles.includes(role)) {
      return true;
    }
    return false;
  });
...
function filterSkills(skills: Skill[], role: RoleType, targetRole?: { prioritySections?: string[] }): Skill[] {
...
export function filterResumeByRole(resume: JSONResume, role: RoleType): JSONResume {
  ...
  if (targetRole?.yearsToInclude) {
    ...
    filteredWork = filteredWork.filter((job) => {
      ...
      const jobYear = parseInt(job.startDate.split('-')[0]);
